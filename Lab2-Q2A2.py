...
n_bits = 6            # Quantizer resolution: 6 bits (64 levels)
...
for n_periods in [30, 100]:
    ...
    window = np.hanning(N)
    x_q_windowed = x_q * window

    # Compute the FFT and PSD of the windowed quantized signal
    X = np.fft.fft(x_q_windowed)
    PSD = (np.abs(X)**2) / (N * fs)
    ...
    plt.ylabel("PSD (dB)")
    plt.title(f"PSD ({n_periods} Periods, Hanning Window Applied)")
    plt.grid(True)
