...
V_rms = 0.2    # RMS value of input signal [V]
...
# Generate sinewave (scaled for 200 mV RMS)
signal = V_rms * np.sqrt(2) * np.sin(2 * np.pi * f * t)
...
# Display results
print(f"SNR (FFT-based): {snr_fft:.2f} dB")

# Plot FFT (Power Spectrum)
plt.figure()
plt.semilogy(xf_pos, power_spectrum)
...
plt.title("Normalized Power Spectrum (PSD / Signal Power)")
plt.xlabel("Frequency [Hz]")
